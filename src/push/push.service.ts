...
export class PushService {
  ...
  // ── Save subscription ──────────────────────────────────────────────
  async subscribe(userId: string, sub: {
    endpoint: string;
  ...
  async unsubscribe(userId: string, endpoint: string) {
    ...
    if (!sub || sub.userId !== userId) throw new NotFoundException('Obuna topilmadi');
  ...
  async sendToHospital(hospitalId: string, payload: PushPayload, roles?: string[]) {
    if (!this.configured) return;
  ...
  private async sendOne(sub: { endpoint: string; p256dh: string; auth: string }, payload: PushPayload) {
    try {
      ...
      if (err?.statusCode === 410 || err?.statusCode === 404) {
        await this.prisma.pushSubscription
          ...
          .catch(() => null);
        this.logger.debug(`Stale push sub removed: ${sub.endpoint.slice(0, 40)}…`);
      } else {
  ...
  async notifyLeaveCreated(hospitalId: string, employeeName: string, leaveType: string) {
    const LEAVE_LABELS: Record<string, string> = {
      VACATION: "Ta'til",  SICK: 'Kasal', PERSONAL: 'Shaxsiy',
      MATERNITY: "Tuğruq ta'tili", UNPAID: 'Haqsiz ta\'til',
    };
    await this.sendToHospital(hospitalId, {
  ...
  async notifyLeaveReviewed(userId: string, decision: 'APPROVED' | 'REJECTED', leaveType: string) {
    const LEAVE_LABELS: Record<string, string> = {
      VACATION: "Ta'til",  SICK: 'Kasal', PERSONAL: 'Shaxsiy',
      MATERNITY: "Tuğruq ta'tili", UNPAID: 'Haqsiz ta\'til',
    };
    ...
    await this.sendToUser(userId, {
      title: decision === 'APPROVED' ? "Ta'til tasdiqlandi ✅" : "Ta'til rad etildi ❌",
  ...
  async notifyPayrollGenerated(userId: string, month: number, year: number, netSalary: number) {
