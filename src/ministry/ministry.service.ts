...
export class MinistryService {
  ...
  async getOverview(date?: string): Promise<MinistryOverview> {
    ...
    const attendanceRate = total > 0
      ? Math.round((totals.todayPresent / total) * 100)
      : 0;

    return {
      totalHospitals:  hospitals.length,
      activeHospitals: hospitals.filter(h => h.isActive && !h.isBlocked).length,
      ...totals,
      attendanceRate,
      hospitals: summaries,
  ...
  async getAbsentToday(hospitalId?: string, date?: string): Promise<AbsentEmployee[]> {
    const today = date
      ? DateUtil.startOfDay(date)
      : DateUtil.startOfDay(new Date());
  ...
  async getPayrollSummary(month: number, year: number): Promise<PayrollSummary> {
    ...
    const hospitalStats = await Promise.all(
      hospitals.map(async h => {
        ...
        const totalBaseSalary = payrolls.reduce((s, p) => s + Number(p.baseSalary), 0);
        const totalNetSalary  = payrolls.reduce((s, p) => s + Number(p.netSalary), 0);
    ...
    const totals = hospitalStats.reduce(
      ...
      { totalBaseSalary: 0, totalNetSalary: 0, totalDeductions: 0, totalBonuses: 0 },
    );
  ...
  private async buildHospitalSummary(
    hospital: { id: string; name: string; code: string; address: string | null; isActive: boolean; isBlocked: boolean },
    today: Date,
  ...
  private async getDepartmentBreakdown(hospitalId: string, today: Date) {
    ...
    return departments.map(dept => {
      ...
      const late     = records.filter(r => ['LATE', 'LATE_EARLY'].includes(r.status)).length;
